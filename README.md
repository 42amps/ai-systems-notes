# AI Systems Notes

This repo collects technical notes around systems I am building and studying: long-horizon agent workflows, RAG/document intelligence, payout correctness, retrieval architecture, evaluation, and product engineering.

## Why This Repo Exists

The goal is to keep a public trail of how I think about applied AI and product systems. These notes are not polished research papers. They are working documents that connect implementation details, system-design tradeoffs, and the projects I am building in public.

## Notes Index

- [Agent State vs Memory](docs/agent-state-vs-memory.md)
- [Payout System Correctness](docs/payout-system-correctness.md)
- [Vision-RAG Notes](docs/vision-rag-notes.md)
- [RAG Evaluation Notes](docs/rag-evaluation-notes.md)
- [Knowledge Graphs for Retrieval](docs/knowledge-graphs-for-retrieval.md)
- [Open Source Contribution Plan](docs/open-source-contribution-plan.md)
- [Open Source Targets](docs/open-source-targets.md)

## Current Focus

![RAG evaluation loop](docs/assets/rag-evaluation-loop.svg)

- Durable state for long-horizon agent workflows.
- Retrieval and document intelligence for layout-heavy files.
- Backend correctness in money-movement simulations.
- Evaluation loops for RAG systems.
- Practical open-source contribution habits.

## Related Projects

- [Stateframe](https://github.com/42amps/stateframe) - file-first task-state ledger for agent workflows.
- [PayRail](https://github.com/42amps/playto-pay-engine) - payout correctness demo with ledger accounting and idempotency.
- [Portfolio](https://github.com/42amps/portfolio) - public project surface for applied AI and product engineering work.

## Status

Active notes repo. Content will evolve as projects, experiments, and contribution work become more concrete.
